/*
 객체의 형변환
   ...
   3. 부모클래스 타입 객체의 주소를 자식클래스타입 객체의 주소로 형변환
       - 자동으로 이루어지지 않는다 (명시적)
       - 원칙적으로는 불가능하다.
       - 부모의 탈을쓴 자식객체(2-3)) 는 가능하다.
       Parent p=new Parent();
       Child cccc = p;(X)
       public class Parent{}
...
public class ParentChildTypeCastingMain {
	...
	public static void main(String[] args) {
		...
		/*
		 * 2. 자식클래스타입 객체의 주소를 부모클래스타입 객체의 주소로 형변환
		 * 	- 자동으로 이루어진다(묵시적)
		 */
		...
		/*
		 * 부모클래스 타입 객체의 주소를 자식클래스타입 객체의 주소로 형변환
       		- 자동으로 이루어지지 않는다.(명시적)
       		- 원칙적으로는 불가능하다.
       		- 부모의 탈을쓴 자식객체(2-3)) 는 가능하다.
		 */
		 ...
		 */
		Parent p3=new Child();
		System.out.println("--Child 객체를 Parent타입변수로 호출--");
		p3.method1();
		p3.method2();
		p3.method3();
		p3.method4();
		//p3.method5();
		Child c3=(Child)p3;
		System.out.println("--Child 객체를 Child타입변수로 호출--");
		c3.method1();
		c3.method2();
		c3.method3();
		c3.method4();
