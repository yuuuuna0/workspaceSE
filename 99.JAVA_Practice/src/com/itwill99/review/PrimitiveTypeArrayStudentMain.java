...
public class PrimitiveTypeArrayStudentMain {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < noArray.length; i++) {
			if (noArray[i] == 3) {
				System.out.printf("%2d %6s %5d %4d %4d %4d %5.1f %3c %4d%n", noArray[i], nameArray[i], korArray[i],
		...
		/*
		 * 정렬 1. 오름차순 ex> 1,2,3,4,5 a,b,c,d,e,.... / 가,나,.....힣 2. 내림차순(descending) ex>
		 * 5,4,3,2,1 z,y,x,....... / 힣,.......,나,가
		 */
		...
		System.out.println("검사 한 횟수:" + count1);

		/*
		 * //case2 int count2=0; boolean b; for (int i = 0; i < noArray.length-1; i++) {
		 * for (int j = 0; j < (noArray.length-1); j++) { b=totArray[j]>totArray[j+1];
		 * if(b) { //swap int tempTot=totArray[j]; totArray[j]=totArray[j+1];
		 * totArray[j+1]=tempTot; } count2=count2+1;
		 * 
		 * } } System.out.println("검사 한 횟수:"+count2);
		 */
