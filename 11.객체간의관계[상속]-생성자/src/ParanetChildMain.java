class Parent extends Object{
	...
	public Parent() {
		/*
		 - 클래스 생성자 첫번째 라인에서는 부모클래스의 생성자가 반드시 호출되어야합니다.
		 - 부모클래스 생성자호출을 생략할 경우에는 부모클래스의 기본생성자가 자동으로 호출된다. --->super(); 생략된 것 
	...
	public Parent(int member1, int member2) {
		...
		System.out.println("1. Object()생성자 호출");
...
class Child extends Parent{
	...
	public Child() {
		/*
		 - 클래스 생성자 첫번째 라인에서는 부모클래스의 생성자가 반드시 호출되어야합니다.
		 - 부모클래스 생성자호출을 생략할 경우에는 부모클래스의 기본생성자가 자동으로 호출된다. 
		 - 매개변수를 가진 부모의 생성자를 호출할수있다.(명시적으로 호출코드를 기술하여야한다.)
		 */
		super();
		System.out.println("3. Child()의 기본 생성자");
	}
	public Child(int member1, int member2, int member3) {
		super(member1,member2);
		/*
		 * <<The filed Parent.member1 is not visible>>
		 * this.member1=member1;
		 * this.member2=member2;
		 */
